...
- U7 ✅（代理本體未驗證：需部署後以真帳號實測）：新增 `api/claude.ts`（POST-only、驗 Supabase JWT、白名單轉發，不進 tsconfig）。`src/services/claude.ts` 抽 `parseClaudeResponse`、`callClaude` 改代理優先：無 session／網路錯誤／404／501／非 JSON → fallback 直呼（localStorage key 路徑原封不動）；2xx → 解析；其他狀態 → 直接 throw body error.message（不 fallback）。`vercel.json` rewrites 排除 `/api`。eslint 加 `api/**/*.ts` node globals 區塊。MenuOverlay 加「已部署雲端代理時此欄可留空」提示。BATCH_SYSTEM_PROMPT／NOTE_ANALYSIS_PROMPT 內容未動。三件套：build 綠、lint 0/0（含 api/）、test 52 passed。`scripts/verify-u7.mjs` 全 PASS：存假 key → 對示範筆記按 AI 解析 → dev 無 /api → fallback → 浮出「AI 解析失敗：Failed to fetch」錯誤 toast、無 pageerror（證明代理 404 → fallback → 錯誤浮出整條鏈不炸）。代理本體無法在本環境實測。commit 231d225 + 本勾銷 commit。
  - **Ryan 手動部署步驟（Opus 不執行）**：Vercel dashboard → 專案 Settings → Environment Variables 新增三個：`ANTHROPIC_API_KEY`（你的 Anthropic key）、`SUPABASE_URL`（同 `VITE_SUPABASE_URL` 的值）、`SUPABASE_ANON_KEY`（同 `VITE_SUPABASE_ANON_KEY` 的值）→ 存檔後 Redeploy。部署後以真帳號登入按「AI 解析」，成功即代表代理生效；此時各裝置 MenuOverlay 的 API key 欄可留空。附註：iOS 對 PWA manifest shortcuts 支援有限（U3），Android 完整。
